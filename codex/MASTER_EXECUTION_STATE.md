...
| 18.5 | CEO Constitution + Managerial Reasoning + Quality Gate | ✅ DONE + LIVE | `docs/JARVIS_CEO_CONSTITUTION.md` (15 principii, v1.0.0) + `constitution.js` + `managerialReasoning.js` (ManagerialAssessment + detectFounderExpectation) + `qualityGate.js` + `managerialClaimValidator.js` (substanta: termen/prag/owner/executie/founder, semantic + diacritice) + `managerialFinalizer.js` (PUNCT UNIC: Assessment→Founder Filter→Claim Validator→Gate→traceability→1 corectie) + `sourcePipeline.js` (root-cause) + founderModel.recordCorrection. Toate canalele trec prin finalizer: chat/Telegram/CEO Home/Ask CODEX/**digest proactiv**/**scheduled reports**. NU poate fi ocolit de canned/fast-path. Validat LIVE (7 intrebari): risc dominant unic (nu TOP 10), founder filter ('JARVIS ii cere Danei, nu tu'), extrase→TECHNICAL_BLOCK (nu rutina manuala), 1.5M→asteptare conditionata (nu fapt), proactiv→'Altfel: tacut'. Teste: constitution (44, TEST A-G) + managerialFinalizer (27 adversariale) |
...
| 28.5 | Action Cards + Envelope + Butoane native | ✅ DONE + LIVE | `src/ceo/actions/`: ActionCard (5 tipuri, 22 never-autonomous), token HMAC semnat, executor 9-pasi idempotent TASKS-only prin CommandBus, revalidare stare→SUPERSEDED, DecisionExample+Preference (context nu situatie→buton), AutonomyPolicy DRAFT→SUPERVISED→ACTIVE (aprobare Adrian obligatorie, never-autonomous nu devine ACTIVE), finalizer {message,action_cards,execution_receipts,policy_references}+assertNoHiddenActions. API /api/actions/*. Gated CEO_ACTION_CARDS_ENABLED. Teste: actionCards (41). Frontend butoane = adaptor (fallback numerotat gata) |
