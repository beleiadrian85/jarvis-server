...
| 28.5 | Action Cards + Autonomie controlata | ✅ DONE (backend) | `src/ceo/actions/`: ActionCard (5 tipuri, 22 never-autonomous), token HMAC semnat, executor 9-pasi idempotent TASKS-only prin CommandBus, revalidare stare→SUPERSEDED, DecisionExample+Preference (context nu situatie→buton), AutonomyPolicy DRAFT→SUPERVISED→ACTIVE (aprobare Adrian obligatorie, never-autonomous nu devine ACTIVE), finalizer {message,action_cards,execution_receipts,policy_references}+assertNoHiddenActions. API /api/actions/*. Gated CEO_ACTION_CARDS_ENABLED. Teste: actionCards (41). Frontend butoane = adaptor (fallback numerotat gata) |
